...
#   2. POST /reset returns a valid observation
...
try:
    ...
    # Must have at least problem_id or metadata
    has_problem_id = bool(obs.get('problem_id'))
    has_meta = bool(obs.get('metadata'))
    has_diff = bool(obs.get('difficulty'))
    has_desc = bool(obs.get('problem_description'))
    ok = has_problem_id or has_meta or has_diff or has_desc
...
# First reset to get a valid state
curl -sf --max-time 10 -X POST "${SERVER_URL}/reset" \
    -H "Content-Type: application/json" -d '{}' >/dev/null 2>&1 || true

STEP_RESP=$(curl -sf --max-time 30 -X POST "${SERVER_URL}/step" \
    -H "Content-Type: application/json" \
    -d '{"action": {"solution_code": "impl Solution { pub fn two_sum(nums: Vec<i32>, target: i32) -> Vec<i32> { vec![] } }", "reasoning_steps": "step-1: Create result vector", "time_complexity": "O(n)"}}' 2>/dev/null || echo "")
...
try:
    data = json.load(sys.stdin)
    obs = data.get('observation', data)
    # Check for grading results in response
    has_reward = data.get('reward') is not None
    has_breakdown = bool(obs.get('reasoning_score')) or bool(obs.get('complexity_score'))
    has_correct = 'correct' in obs
    has_grading = has_reward or has_breakdown or has_correct
